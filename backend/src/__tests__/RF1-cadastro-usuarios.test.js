...
const { createUser, validateCPF, validateEmail, validatePassword, validateCNH } = require('../services/authService');
const { getUserByEmail, getUserByCPF, getUserByCNH, clearDatabase } = require('../services/userService');
...
describe('RF1: Cadastro de Usuários', () => {
  ...
  describe('Validação de CNH', () => {
    ...
    it('deve aceitar CNH com até 12 caracteres', () => {
      expect(validateCNH('123456789012', { validateInProduction: false })).toBe(true);
    });
  });
  ...
  describe('Cenário 1.6: Senha Fraca', () => {
    ...
    const weakPasswords = [
      { password: 'abc123', reason: 'sem letra maiúscula e sem caractere especial' },
  ...
  describe('Cenário 1.8: Campos Obrigatórios Vazios - Passageiro', () => {
    ...
    requiredFields.forEach(field => {
      it(`deve rejeitar cadastro com campo obrigatório vazio: ${field}`, async () => {
        ...
        expect(result.success).toBe(false);
        expect(result.error.message).toBe('Preencha todos os campos obrigatórios');
  ...
  describe('Cenário 1.9: Campos Obrigatórios Vazios - Motorista', () => {
    ...
    const requiredFields = ['name', 'cpf', 'cnh', 'birthYear', 'email', 'password'];
